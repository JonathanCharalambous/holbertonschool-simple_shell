...
|[`parse_line.c`](https://github.com/JonathanCharalambous/holbertonschool-simple_shell/blob/main/parse_line.c) | `void parse_line(char *line, char **arg);` | _split a line into words (arguments)_ - This function takes a line of text (like a command line) and splits it into separate words using strtok() with DELIM as the separator. <br> Each word is saved in the arg array. It keeps splitting until there are no more words or until it reaches the maximum number of arguments. The last element of arg is set to NULL to mark the end. |
